...
def depth_first_search(start_state, goal_state, forbidden=[]):
    ...
    while current_node.value != goal_state:
        ...
        for unexpanded in fringe:
            ...
            if unexpanded.value in expanded_map.keys():
                ...
                if unexpanded.changed not in expanded_map[unexpanded.value]:
                    # make this the current node and stop iterating through the fringe
                    current_node = unexpanded
                    fringe.remove(unexpanded)
                    break
            else:
